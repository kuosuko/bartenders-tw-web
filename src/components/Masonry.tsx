...
const Masonry: React.FC<MasonryProps> = ({
  ...
  const handleImageClick = (item: Item) => {
    const target = item.url || item.img;
    const resolved = target.includes('-400x300') ? target.replace('-400x300', '-768x512') : target;
    ...
    <>
      ...
      >
        {grid.map(item => (
          ...
          >
            <div
              className="relative w-full h-full overflow-hidden rounded-[10px] shadow-[0px_10px_50px_-10px_rgba(0,0,0,0.2)] bg-zinc-900 border border-white/10 transition-colors"
            >
              <img 
                src={item.img} 
                alt=""
